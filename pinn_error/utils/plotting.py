...
- Error bound comparison over time (heat equation)
"""
...
def plot_error_bound_comparison(
    ...
    Compare error measures over time for the 1D heat equation:
        ...
        e_true : true PINN error                    ||u_exact - u_pinn||_L2
        e_res  : our residual-integration estimate  ||e_res||_L2
        e_FDM  : FDM-based estimate                 ||u_fdm - u_pinn||_L2
        e_HU   : Hillebrecht & Unger semigroup upper bound (results['epsilon'])

    All curves are spatial L2 norms evaluated at each time step.

    Parameters
    ----------
    results : dict
        Results from ExperimentFactory.run_experiment(), updated with the
        output of PINNErrorBoundEstimator.compute() (so that 'epsilon' and
        't' are present).
    bound_estimator : PINNErrorBoundEstimator
        Used for its spatial L2 norm quadrature (and its run_time for the
        timing table), so the norms match the ones used inside the bound.
    constraint_mode : str
        'hard', 'soft_ic' or 'soft_full'. Recorded in the title and used to
        build the default filename, so runs with different constraint modes
        don't overwrite each other.
    save_path : str, optional
        Where to write the figure. Parent directories are created if needed.
        Defaults to './figures/heat_eq_err_comparison_time_{constraint_mode}.pdf'.
    show : bool
        Whether to display the figure.
    title : str, optional
        Overrides the default axis title.
    ...
    Notes
    -----
    The Hillebrecht & Unger bound assumes the PINN satisfies the *boundary*
    conditions exactly (its semigroup is generated by the Dirichlet
    Laplacian). That holds for constraint_mode 'hard' and 'soft_ic', which
    both hard-constrain the BC. For 'soft_full' the boundary error is
    nonzero and unaccounted for in the theory, so e_HU is indicative rather
    than a rigorous upper bound -- the plot is annotated accordingly.
    """
    t = np.asarray(results["t"])

    # Spatial L2 norm at each time step, using the estimator's own quadrature
    def _l2_over_time(field) -> np.ndarray:
        field = np.asarray(field)
        return np.array(
            [bound_estimator._compute_spatial_l2_norm(field[i].flatten())[0]
             for i in range(len(t))]
        )

    e_true = _l2_over_time(results["e_true"])
    e_res = _l2_over_time(results["e_res"])
    e_fdm = _l2_over_time(
        np.asarray(results["u_fdm"]) - np.asarray(results["u_pinn"])
    )

    fig, ax = plt.subplots(figsize=(9, 6))
    ax.plot(t, e_true, label=r"$e_{\mathrm{true}}$", color="#4daf4a", lw=2.0, alpha=0.5)
    ax.plot(t, e_res, label=r"$e_{\mathrm{res}}$", color="#377eb8", linestyle="--", lw=1.5)
    ax.plot(t, e_fdm, label=r"$e_{\mathrm{FDM}}$", color="#ff7f00", linestyle="-", lw=1.5)
    if "epsilon" in results:
        ax.plot(
            t, results["epsilon"], label=r"$e_{\mathrm{HU}}$",
            color="#f781bf", linestyle="-.", lw=1.5,
        )
    ...
    ax.text(
        0.55, 0.05, metrics_text,
        verticalalignment="bottom", horizontalalignment="left",
        transform=ax.transAxes,
        bbox=dict(edgecolor="black", alpha=0.8, facecolor="white"),
    )

    if title is None:
        title = f"Error Comparison for 1D Heat Equation ({constraint_mode})"
    ax.set_title(title)
    ax.legend()

    if constraint_mode == "soft_full" and "epsilon" in results:
        # be explicit on the figure itself that the bound isn't rigorous here
        ax.text(
            0.02, 0.02,
            "note: BC soft-constrained,\n$e_{\\mathrm{HU}}$ not a rigorous bound",
            transform=ax.transAxes, fontsize=8, verticalalignment="bottom",
            bbox=dict(edgecolor="none", facecolor="white", alpha=0.6),
        )

    # NOTE: legend/title are set BEFORE saving -- previously savefig ran first,
    # so the written PDF had neither.
    if save_path is None:
        save_path = f"./figures/heat_eq_err_comparison_time_{constraint_mode}.pdf"
    parent = os.path.dirname(save_path)
    if parent:
        os.makedirs(parent, exist_ok=True)  # savefig won't create it itself
    fig.savefig(save_path, bbox_inches="tight")
    print(f"Error bound figure saved to: {save_path}")
